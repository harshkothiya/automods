...
function dactor_setup() {
	
	//Register a theme image size.
	add_theme_support( 'post-thumbnails' );
	add_image_size( 'dactor-large-thumb', 830 );
	add_image_size( 'dactor-medium-thumb', 550, 400, true );
	add_image_size( 'dactor-small-thumb', 230 );
	
	//Register nav menu used in theme.
	register_nav_menus( array(
		'primary' 	=> __( 'Primary Menu', 'dactor' )
	) );
...
add_action( 'after_setup_theme', 'dactor_setup' );
...
add_action('dactor_before_site', 'dactor_preloader');
...
function dactor_grid_post_meta(){
...
function dactor_excerpt_more($more) {
	global $post;
	$layout = theme_blog_layout();
	return ' ....&nbsp;&nbsp;<a class=" '.$layout.'" href="'. get_permalink($post->ID) . '">' . __('Read More', 'dactor') . '</a>';
}
add_filter('excerpt_more', 'dactor_excerpt_more');


/*--- Enqueue scripts and styles ---*/
function dactor_scripts() {
	...
	wp_enqueue_script( 'dactor-common', get_template_directory_uri() . '/js/common.js', array('jquery'), '20180213', true );
	...
	wp_enqueue_style( 'dactor-font-awesome', get_template_directory_uri() . '/fonts/font-awesome.min.css' );

	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}
	
	/* wp_enqueue_style( 'dactor-bootstrap', get_template_directory_uri() . '/css/bootstrap/bootstrap.min.css', array(), true ); */
	
}
add_action( 'wp_enqueue_scripts', 'dactor_scripts' );

/**
 * Enqueue Bootstrap
 */
function dactor_enqueue_bootstrap() {
	wp_enqueue_style( 'dactor-bootstrap', get_template_directory_uri() . '/css/bootstrap/bootstrap.min.css', array(), true );
}
add_action( 'wp_enqueue_scripts', 'dactor_enqueue_bootstrap', 9 );



/*--- Register widget area ---*/
add_action( 'widgets_init', 'dactor_widgets_init' );
function dactor_widgets_init() {
    register_sidebar( array(
        'name' => __( 'Main Sidebar', 'dactor' ),
        'id' => 'sidebar-1',
        'description' => __( 'Widgets in this area will be shown on all posts and pages.', 'dactor' ),
	...
	for ($i=1; $i<=3; $i++){
		register_sidebar( array(
			'name' => __( 'Footer-widget-area-', 'dactor' ) . $i,
			'id' => 'footer-widget-area' . $i,
			'description' => __( 'Widgets in this area will be shown on footer.', 'dactor' ),
			'before_widget' => '<li id="%1$s" class="widget %2$s">',
			'after_widget'  => '</li>',
			'before_title'  => '<h3 class="widgettitle">',
			'after_title'   => '</h3>',
...
function dactor_edit_link() {
	edit_post_link(
		sprintf( __( '<i class="fa fa-edit"></i>Edit<span class="screen-reader-text"> "%s"</span>', 'dactor' ), get_the_title() ),
...
function dactor_get_breadcrumb() {
    echo '<a href="'.home_url().'" rel="nofollow">'.__('Home', 'dactor').'</a>';
    if ( is_category() || is_single() ) {
        ...
        echo __("&nbsp;&nbsp;&#47;&nbsp;&nbsp;Search Results for... ", "dactor");
        echo '"<em>';
...
function dactor_recommended_plugin() {
...
add_action( 'tgmpa_register', 'dactor_recommended_plugin' );
...
add_filter( 'pre_set_site_transient_update_themes', 'dactor_check_for_update' );
function dactor_check_for_update( $checked_data ){
...
popularfx_maybe_promo([
	'after' => 1,// In days
	'interval' => 30,// In days
	'pro_url' => 'https://popularfx.com/themes/wordpress/corporate/Dactor_Pro',
	'rating' => 'https://popularfx.com/themes/wordpress/corporate/Dactor',
	'twitter' => 'https://twitter.com',
	'facebook' => 'https://www.facebook.com/',
	'website' => 'http://suavethemes.com',
]);

function dactor_upsell_script() {
...
add_action( 'customize_controls_enqueue_scripts', 'dactor_upsell_script' );